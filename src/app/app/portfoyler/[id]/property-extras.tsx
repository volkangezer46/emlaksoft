...
import {
  Clock, FileCheck2, History, Plus, RadioTower, Send, Shield, X,
} from "lucide-react";
import {
...
export function PublishToPortalsPanel({
  ...
  return (
    <section className="rounded-[20px] border border-line bg-surface p-5">
      ...
      <p className="mt-1 text-xs text-text-muted">API entegrasyonu yapılandırılmış portallere tek tıkla gönder.</p>
          ...
          return (
            <div key={p.name} className="flex items-center gap-2">
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-[9px] border border-line bg-canvas px-3 py-2 text-xs font-semibold text-brand-600 transition hover:bg-brand-600/5 hover:border-brand-300"
              >
                <Send className="h-3.5 w-3.5" /> {p.label}
              </button>
              {res?.ok   && <span className="text-[10px] font-bold text-mint-600">✓ Gönderildi</span>}
              {res?.error && <span className="max-w-[140px] text-[10px] text-red-600 truncate">{res.error}</span>}
            </div>
          );
        })}
      </div>
