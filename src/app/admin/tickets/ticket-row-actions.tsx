...
function PillSelect({
  ...
  return (
    <label
      className={`focus-ring group relative inline-flex min-w-0 items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-bold transition ${wrapCls} ${pending ? "opacity-60" : ""}`}
    >
      {leading}
      <select
        name={name}
