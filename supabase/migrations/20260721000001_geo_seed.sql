...
select istanbul.id, d from istanbul, unnest(array[
  'Kadıköy','Üsküdar','Beşiktaş','Şişli','Bakırköy','Maltepe','Ataşehir','Başakşehir',
  'Beylikdüzü','Pendik','Kartal','Ümraniye','Sarıyer','Fatih','Beyoğlu','Eyüpsultan'
]) as d
...
select ankara.id, d from ankara, unnest(array[
  'Çankaya','Keçiören','Yenimahalle','Etimesgut','Mamak','Sincan','Altındağ','Gölbaşı','Pursaklar'
]) as d
on conflict (province_id, name) do nothing;
...
select izmir.id, d from izmir, unnest(array[
  'Konak','Karşıyaka','Bornova','Buca','Bayraklı','Çiğli','Gaziemir','Karabağlar','Balçova','Narlıdere'
]) as d
on conflict (province_id, name) do nothing;
